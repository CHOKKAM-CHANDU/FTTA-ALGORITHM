...
def ftta_algorithm(tasks, deadlines, num_vms):
    ...
    results = []
    header = f"<tr><th>Step</th><th>Ready list</th><th>Task selected</th><th>Execution time</th><th>Deadline</th><th>VM selected</th><th>Busy VMs</th></tr>"
    ...
    while ready_list:
        ...
        # Record busy VMs at this step
        busy_vms_info = [f"VM{idx + 1}" for idx, busy in enumerate(busy_vms) if busy and idx != min_vm_index]
        if busy_vms_info:
        ...
        row = f"<tr><td>{step}</td><td>{ready_list_str}</td><td>T{task_index}</td><td>{exec_time_str}</td><td>{deadline}</td><td>{vm_selected}</td><td>{busy_vms_str}</td></tr>"
        results.append(row)
        step += 1
